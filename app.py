...
"""
    ...
    # AI 제안
    # ========================
    st.subheader("② AI 제안 (분류/치료/기간/주의사항/추가 제안)")
    plan_prompt = f"""
너는 한의원 상담 보조 도우미다.
환자 문진을 보고 JSON으로만 출력하라.

필수 필드:
- classification: 급성/만성/웰니스
- duration: 치료 기간 (1주,2주,3주,4주,1개월 이상)
- covered: {COVERED_ITEMS} 중 선택
- uncovered: {UNCOVERED_ITEMS} 중 선택
- rationale: 권장 이유
- objective_comment: 생활습관/재발예방 등 참고
- caution: 복용약/병력 고려한 주의사항 (없으면 "특이사항 없음")
- extra_suggestion: 내가 준 카테고리에 없는 치료나 관리법 제안 (없으면 "없음")

문진 내용:
{patient_data}
"""
    ai_plan_raw = call_ai(plan_prompt)

    import json, re
    try:
        m = re.search(r"\{[\s\S]*\}", ai_plan_raw)
        parsed = json.loads(m.group(0)) if m else {}
    except:
        parsed = {}

    st.write(ai_plan_raw)
...
"""
        ...
        st.button("복사하기", on_click=lambda: copy_to_clipboard(final_text))
